...
describe('server api', () => {
    ...
    it('responds to a request for one item', async (done) => {
        const response = await request.get('/api/4');
        expect(response.statusCode).toBe(200);
        done();
    })

    it('responds to a request to create items', async (done) => {
        const response = await request.post('/api');
        expect(response.statusCode).toBe(201);
        done();
    })

    it('responds to a request to update an item', async (done) => {
        const response = await request.put('/api/4');
        expect(response.statusCode).toBe(202);
        done();
    })

    it('responds to a request to delete one item', async (done) => {
